...
class Sentry {
	...
	configPlugin() {
		...
		if (!_.has(this.sentry, "url")) {
			this.sentry.url = "https://sentry.io";
		}
	}
	...
	createSentryRelease() {
		...
		return BbPromise.fromCallback(cb =>
			request.post(`${this.sentry.url}/api/0/organizations/${organization}/releases/`)
	...
	deploySentryRelease() {
		...
		return BbPromise.fromCallback(cb =>
			request.post(`${this.sentry.url}/api/0/organizations/${organization}/releases/${encodeURIComponent(release.version)}/deploys/`)
				.set("Authorization", `Bearer ${this.sentry.authToken}`)
				.send({
